This tutorial contains instructions for compiling and running the SimCov immunology model on the Easley cluster.
...
Load Easley modules and set UPCXX variables (NOTE: modules subject to change use 'module spider' to find availability):
```
...
## Submit a SimCov Job
A Easley PBS script is provided for you. We have submitted the script below to the simcov developers - so hopefully by the time you pull simcov the code below will already be in the easley_simcov_run.pbs. If not update the script to contain the following:
