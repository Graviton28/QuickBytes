...
Log into a CARC shared cluster (we’ll be using Easley.carc.unm.edu) using ssh or a similar tool (XXX This should link to our documentation on how to do that)
...
1. Create an interactive 4-node cluster on Easley for 1 hour

```bridges@easley-sn[519]> ls big.txt  conf/  pbs-spark-submit*  small.txt
bridges@easley-sn[520]> cd
bridges@easley-sn[521]> qsub -I -l nodes=4:ppn=8 -l walltime=01:00:00
qsub: waiting for job 14319.easley-sn.alliance.unm.edu to start
qsub: job 14319.easley-sn.alliance.unm.edu ready
Job 14319.easley-sn.alliance.unm.edu running on nodes:
easley240 easley235 easley234 easley233
Easley Portable Batch System Prologue
Job Id: 14319.easley-sn.alliance.unm.edu
Username: bridges
Job 14319.easley-sn.alliance.unm.edu running on nodes:
easley240 easley235 easley234 easley233
prologue running on host: easley240
```

2. Start the Spark cluster on your Easley nodes

```bridges@easley240[500]> cd /easley/scratch/bridges/spark
bridges@easley240[501]> module load spark-2.1.0-gcc-4.8.5-lifnga6
bridges@easley240[502]> export SPARK_HOME=$SPARK_DIR
bridges@easley240[503]> ./pbs-spark-submit
SPARK_MASTER_HOST=easley240
SPARK_MASTER_PORT=7077
bridges@easley240[504]> pyspark --master spark://easley240:7077
```
...
>>> exit()
bridges@easley240[505]> exit
qsub: job 14319.easley-sn.alliance.unm.edu completed
```
4. Spark jobs can be run in batch as well – The following PBS batch script would run the simple wordcount example from above, using the Easley scratch file system to store the data while it is being processed:

```# wordcount.pbs
#PBS -N wordcount
#PBS -l nodes=2:ppn=8
#PBS -l walltime=00:05:00
SCRATCHDIR=/easley/scratch/$USER/spark
...
To make the most of Spark, most programmers will want to do two things: use the DataFrames API and construct a more full-featured Python programming environment in which to work and analyze data. While RDDs are the core Spark abstraction, the DataFrame API (available since Spark 2.0) is much more programmer friendly. In particular, DataFrames have columns and column operations, can import CSV and JSON data files, while still being distributed across the memory of all of the nodes in your Spark cluster. 16 Easley nodes can, for example, handle a 200GB table! Similarly, setting up a full-featured programming environment by using Anaconda will allow you to analyze and visualize your data much more effectively.
...
```easley-sn> module load anaconda3-4.4.0-gcc-4.8.5-ubgzkkv
easley-sn> conda create -n spark python=27 numpy scipy pandas matplotlib
```
...
```easley-sn> qsub -I -l nodes=1:ppn=8 -l walltime=01:00:00
easley263> module load anaconda3-4.4.0-gcc-4.8.5-ubgzkkv
easley263> source activate spark
(spark) easley263> module load spark spark-2.1.0-gcc-4.8.5-lifnga6
(spark) easley263> export SPARK_HOME=$SPARK_DIR
(spark) easley263> ./pbs-spark-submit
(spark) easley263> pyspark --master spark://easley263:7077
```
